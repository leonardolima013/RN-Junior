...
# Selecionar cangaceiros e empresas a serem mostrados
cangaceiros_selecionados = st.multiselect(
    "Selecione os Cangaceiros para exibir:",
    options=match_matrix.index.tolist(),
    default=match_matrix.index.tolist()
)

empresas_selecionadas = st.multiselect(
    "Selecione as Empresas para exibir:",
    options=match_matrix.columns.tolist(),
    default=match_matrix.columns.tolist()
)

# Filtrar a matriz com base na seleção
filtered_matrix = match_matrix.loc[cangaceiros_selecionados, empresas_selecionadas]
...
fig = ff.create_annotated_heatmap(
    z=filtered_matrix.values.astype(float),
    x=[
        " ".join(name.split()[:2]) if len(name.split()) > 1 else name
        for name in filtered_matrix.columns
    ],
    y=filtered_matrix.index.tolist(),
    colorscale='YlGn',
    annotation_text=filtered_matrix.values.astype(str)
)
...
if st.button("Baixar Matriz em Excel"):
    st.download_button(
        label="Download",
        data=convert_to_excel(filtered_matrix).path,
        file_name="match_matrix_filtered.xlsx",
